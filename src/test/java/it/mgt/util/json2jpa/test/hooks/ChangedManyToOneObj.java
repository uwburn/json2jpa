...
public class ChangedManyToOneObj {

    private static final Logger logger = LoggerFactory.getLogger(ChangedManyToOneObj.class);
    ...
    @Transactional
    public void test()  {
        logger.info("Testing changed many-to-one object property");

        Flag handlerInvoked = new Flag(false);

        ObjectNode json = objectMapper.createObjectNode();
        ObjectNode countryJson = json.putObject("country");
        countryJson.put("id", landOfOz.getId());
        countryJson.put("name", landOfOz.getName());
